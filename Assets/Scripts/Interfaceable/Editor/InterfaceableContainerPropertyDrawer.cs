...
using NeverLab.Interfaceable.Editor;
using UnityEditor;
using UnityEngine;
...
[CustomPropertyDrawer(typeof(NeverLab.Interfaceable.InterfaceableContainerBase), true)]
public class InterfaceableContainerPropertyDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if(containerType == null)
        {
            ...
            position.height -= 2;
            position.y += 1;
            GUI.Label(position, $"Cannot draw '{property.name}'.", InterfaceableGUIHelper.InspectorStyles.Error);
            return;
        }
    ...
    private static InterfaceableContainerSelectWindow _selectWindow;
    ...
    private static string BuildEditorResultString(string resultType, Object @object)
    {
        if(@object != null)
        {
            ...
            if(component != null)
            {
                return $"{component.gameObject.name} ( {NeverLab.Interfaceable.InterfaceableContainerBase.ConstructResolvedName(@object.GetType())} )";
            }

            return InterfaceableGUIHelper.GetObjectName(@object);
        }
    ...
    private static IEnumerable<SelectableObject> GetSelectable<TResult>(bool projectAssetsOnly)
        where TResult : class
    {
        var objects = InterfaceableGUIHelper.EnumerateSavedObjects().Concat(projectAssetsOnly ? new Object[0] : Object.FindObjectsOfType<Object>());
	...
	/// <summary>
	/// Given a SerializedProperty of a field, List, or array of InterfaceableContainer&lt;T&gt; derivative(s), will initialize null references and return the container derivative type.
	/// </summary>
	/// <returns>The type deriving from InterfaceableContainer&lt;T&gt;, if any - null otherwise.</returns>
    ...
    private static Type GetContainerType(SerializedProperty property)
    {
        ...
        if(IsSubclassOfRawGeneric(type, typeof(InterfaceableContainer<>), out genericArguments))
        {
    ...
    private static readonly MethodInfo DrawMethodInfo = CachedType<InterfaceableContainerPropertyDrawer>.Type.GetMethod("DrawInterfaceableContainer", BindingFlags.Static | BindingFlags.NonPublic);
	...
	/// <summary>
	/// Handy class wrapping around an InterfaceableContainer-derived serialized property.
	/// </summary>
	public class SerializedContainer
    {
        ...
        {
            get { return InterfaceableGUIHelper.IsProjectAsset(_containerProperty.serializedObject.targetObject); }
        }
